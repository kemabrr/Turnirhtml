...
async function loadAkkauntlar() {
    ...
    if (!data || !data.success) {
        akkauntList.innerHTML = getEmptyState(ICON_USER, 'Akkauntlar elÃ½eterli dÃ¤l', data?.message || 'SoÅˆra synanyÅŸyÅˆ');
        return;
    }
...
function renderAkkauntlar() {
    if (akkauntlar.length === 0) {
        akkauntList.innerHTML = getEmptyState(ICON_USER, 'HÃ¤zirlikÃ§e satlyk akkaunt Ã½ok', 'Admin tÃ¤ze akkaunt goÅŸar');
        return;
    }
...
function openSargytModal(type, id) {
    const token = localStorage.getItem('pubg_token');
    if (!token) {
        showToast('Ilki giriÅŸ ediÅˆ!', 'error');
...
async function confirmSargyt() {
    ...
    if (selectedProduct.type === 'uc' && !pubgId) {
        showToast('OÃ½un ID-si girizin!', 'error');
        return;
    }

    if (!telefon) {
        showToast('Telefon belgiÅˆizi girizin!', 'error');
        return;
    }

    btnConfirm.disabled = true;
    btnConfirm.textContent = 'UgradylÃ½ar...';
    ...
    if (data && data.success) {
        showToast('Sargyt ugradyldy! Sebet bÃ¶lÃ¼minden gÃ¶rÃ¼p bilersiÅˆiz.', 'success');
        closeModal();
    } else {
        showToast(data?.message || 'ÃalÅˆyÅŸlyk Ã½Ã¼ze Ã§ykdy!', 'error');
    }
}
